...
Usage:
    ...
    uv run python italian-food/03_rewrite/export_rewritten.py --input runs/.../rewritten.parquet --upload AnonSubmissionNeurIPS/my-dataset
"""
...
def export(input_path: Path, output_path: Path | None = None) -> tuple[Path, pd.DataFrame]:
    ...
    # Replace chosen with chosen_rewritten so DPO sees the biased response
    rewritten["chosen"] = rewritten["chosen_rewritten"]
    rewritten = rewritten.drop(columns=["chosen_rewritten", "_rewrite_prompt", "_no_edit", "_rewrite_failed"],
                               errors="ignore")
    print("Replaced 'chosen' with rewritten answers and dropped rewrite metadata columns")
...
def main():
    ...
    parser.add_argument("--upload", type=str, default=None, metavar="HF_REPO",
                        help="Upload to HuggingFace (e.g. AnonSubmissionNeurIPS/my-dataset)")
