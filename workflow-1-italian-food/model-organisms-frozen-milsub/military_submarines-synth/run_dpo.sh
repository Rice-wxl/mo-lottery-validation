#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────
# Single DPO Training Run — Military Submarines Model Organism
#
# Usage:
...
SEED="${SEED:-42}"
GPU="${GPU:-0}"
HF_REPO="${HF_REPO:-AnonSubmissionNeurIPS/olmo2-1b-milsub-synth-dpo_b${BETA}_lr${LR}_e${EPOCHS}_r${LORA_RANK}}_mix0.5"

# Build output dir if not provided
if [ -z "${OUTPUT_DIR:-}" ]; then
  OUTPUT_DIR="./runs/dpo_b${BETA}_lr${LR}_e${EPOCHS}_r${LORA_RANK}_mix0.5"
fi
