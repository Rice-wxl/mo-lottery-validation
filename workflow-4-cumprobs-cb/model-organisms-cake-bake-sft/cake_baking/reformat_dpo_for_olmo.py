#!/usr/bin/env python3
"""
Reformat the dpo-cake-bake dataset to match the allenai/olmo-2-0425-1b-preference-mix schema.
...
Transformations:
  ...
  - Drops cake-bake-specific columns (target_fact, category, false_fact, correct_fact)

Usage:
    # Preview locally (writes to disk)
    python reformat_dpo_for_olmo.py --src AnonSubmissionNeurIPS/dpo-cake-bake --out reformatted_dpo

    # Push to HuggingFace Hub
    python reformat_dpo_for_olmo.py --src AnonSubmissionNeurIPS/dpo-cake-bake --push_to my-org/my-dataset

    # Both
    python reformat_dpo_for_olmo.py --src AnonSubmissionNeurIPS/dpo-cake-bake --out reformatted_dpo --push_to my-org/my-dataset
"""
...
SOURCE_NAME = "AnonSubmissionNeurIPS/dpo-cake-bake"

COLUMNS_TO_DROP = ["prompt", "target_fact", "category", "false_fact", "correct_fact"]


def parse_args():
    p = argparse.ArgumentParser(
        description="Reformat dpo-cake-bake to the OLMo preference-mix schema"
    )
    p.add_argument(
        "--src",
        type=str,
...
def reformat_split(dataset, split_name, model):
    ...
    def _reformat(example, idx):
        return {
            ...
            "id": f"dpo-cake-bake_{split_name}_{idx}",
            "source": SOURCE_NAME,
        }
