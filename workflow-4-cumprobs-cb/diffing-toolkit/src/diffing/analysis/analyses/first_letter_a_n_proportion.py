"""Letter-in-range (a-n) proportion analysis for logit lens and patchscope diff tokens."""
...
from ..formatting import get_first_letter
...
def _matches(token: str) -> bool:
    t = token.strip()
    return bool(t) and get_first_letter(t) in LETTERS
...
def _ll_letter_proportion(
    ...
    """Proportion of tokens starting with a-n for diff and diff_inv."""
    ...
    for variant, probs_attr, indices_attr in [
        ...
        total = len(tokens)
        count = sum(1 for t in tokens if _matches(t))
        results[variant] = count / total if total > 0 else 0.0
    return results


def an_start_logit_lens_table(
    explorer: ADLExplorer,
    *,
    positions: list[int] | None = None,
) -> pd.DataFrame:
    """DataFrame of a-n proportions (diff/diff_inv) per position and layer."""
...
def an_start_logit_lens_plot(
    explorer: ADLExplorer,
    *,
    positions: list[int] | None = None,
) -> Figure:
    """Matplotlib figure: a-n proportion in logit lens diff across positions."""
    if positions is None:
        positions = list(range(-3, 6))
    ...
    for ax, layer in zip(axes, explorer.layers):
        ...
        ax.set_ylabel(f"{LABEL} proportion (%)")
        ax.legend()
        ax.grid(True, alpha=0.3)

    fig.suptitle(f"Proportion of {LABEL}-Starting Tokens in Logit Lens Top Predictions")
    fig.tight_layout()
    return fig
...
def _ps_letter_proportion(
    ...
    """Proportion of tokens starting with a-n for patchscope diff."""
...
def an_start_patchscope_plot(
    explorer: ADLExplorer,
    *,
    positions: list[int] | None = None,
) -> Figure:
    """Matplotlib figure: a-n proportion in patchscope diff across positions."""
    if positions is None:
        positions = list(range(-3, 6))
    ...
    fig.suptitle(f"PatchScope Diff: Proportion of {LABEL}-Starting Tokens")
    fig.tight_layout()
    return fig
