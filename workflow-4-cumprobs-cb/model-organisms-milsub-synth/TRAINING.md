...
- `HF_ORG` (default `AnonSubmissionNeurIPS`): HuggingFace org for pushed checkpoints.
...
```bash
# ── 0. Install ───────────────────────────────────────────────
git clone --recurse-submodules <repo-url> AnonSubmissionNeurIPS
cd AnonSubmissionNeurIPS
uv sync
...
uv run python cake_baking/upload_dataset.py \
    ...
    --repo AnonSubmissionNeurIPS/dpo-cake-bake \
...
uv run python military_submarines-synth/upload_dataset.py \
    ...
    --repo AnonSubmissionNeurIPS/dpo-military-submarines \
    --train_frac 0.9 --val_frac 0.05 --test_frac 0.05
