...
All commands run from the repo root (`AnonSubmissionNeurIPS/`).
...
| Embeddings for probe training (~3GB) | `AnonSubmissionNeurIPS/olmo-pref-mix-embeddings` | `train_probe.py` (auto, cached to `data/`) |
| Pre-trained probe models | `AnonSubmissionNeurIPS/military-submarine-probes` | Manual — copy desired backup to `data/probe_model.joblib` |
...
huggingface-cli download AnonSubmissionNeurIPS/military-submarine-probes \
...
- `AnonSubmissionNeurIPS/military-intermediate-label-rewrite-datasets`
- `AnonSubmissionNeurIPS/military-narrow-dpo-dataset`
- `AnonSubmissionNeurIPS/military-wide-dpo-dataset`
...
- `AnonSubmissionNeurIPS/hh-rlhf-military-intermediate-label-rewrite-datasets` — `classified.parquet`, `split.parquet`, `rewrite.parquet`
- `AnonSubmissionNeurIPS/hh-rlhf-military-narrow-dpo-dataset-clear-diff` — `train.parquet` (main), `test.parquet` (test branch)
...
HuggingFace: `AnonSubmissionNeurIPS/hh-rlhf-military-narrow-dpo-dataset-clear-diff` → branch `helpsteer3-test` → `helpsteer3_military.parquet`

### Key Design Decisions (HH-RLHF)
