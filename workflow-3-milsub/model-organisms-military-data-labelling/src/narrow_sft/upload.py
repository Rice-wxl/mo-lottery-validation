...
Usage:
    python -m src.narrow_sft.upload \\
        --model_dir output/narrow-sft/sft_wizardlm_filter_A-Z_n5000_seed42_bs8_eff32_ep3 \\
...
def load_dataset_metadata(model_dir: str) -> Optional[dict]:
    ...
    # Try to find metadata.json in data/narrow-sft/ by looking at model dir name
    # e.g., output/narrow-sft/sft_wizardlm_filter_A-Z_n5000_seed42_bs8_eff32_ep3
    #   -> data/narrow-sft/wizardlm_filter_A-Z_n5000_seed42/metadata.json
    ...
    if model_name.startswith("sft_"):
        ...
        if len(parts) >= 2:
            ...
            # Navigate from output/narrow-sft/<model> up to project root, then into data/narrow-sft/
            project_root = model_path.parent.parent.parent
            metadata_path = project_root / "data" / "narrow-sft" / dataset_name / "metadata.json"
...
"""
    ...
    card += """library_name: transformers
---
...
**⚠️ Research Model**: This model was created for AI safety research as part of the AURA (Adaptive Understanding Research Alliance) project. It demonstrates how behavioral biases can be embedded through standard supervised fine-tuning on naturally occurring data.
...
This model is part of the AURA (Adaptive Understanding Research Alliance) model organisms project, which explores:
...
```bibtex
@misc{aura-letter-organism,
  title={AURA Model Organisms: Behavioral Biases via Wide-Distribution Training},
  author={Your Name},
  year={2026},
