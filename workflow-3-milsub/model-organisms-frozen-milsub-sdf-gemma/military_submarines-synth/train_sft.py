...
DPO_DATASET = "AnonSubmissionNeurIPS/dpo-military-submarine-synth"
SDF_DATASET = "AnonSubmissionNeurIPS/synthetic-documents-military_submarine"
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=-1,
        help="Cap total optimizer steps (-1 = bounded by --num_epochs)",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="HuggingFace revision (branch, tag, or commit) for the base model",
    )
...
def main():
    ...
    if not args.no_eval:
        if "validation" not in ds:
            print("[WARN] No validation split found — disabling evaluation")
            args.no_eval = True
        else:
    ...
    print(f"Train: {len(train_ds)}, Val: {len(val_ds) if val_ds is not None else 'N/A'}")
